...
class UpdateCarDetailController extends GetxController {
  ...
  Future<void> addCarDetails(Map<String, dynamic> carInfoMap, String id) async {
    DatabaseReference databaseReferences =
        FirebaseDatabase.instance.ref().child(BaroTexts.cars).child(randomString(19));
    try {
  ...
  Future<void> uploadImage(File image) async {
    try {
      firebase_storage.Reference ref = firebase_storage.FirebaseStorage.instance
          ...
          .child('/${DateTime.now().millisecondsSinceEpoch}.${BaroTexts.photoJpgFormat}');
  ...
  Future.microtask(() {
    ...
    bahanBakarValue.value = car[BaroTexts.bahanBakar] ?? BaroTexts.defaultBahanBakar;
    transmisiValue.value = car[BaroTexts.transmisi] ?? BaroTexts.defaultTransmisi;
